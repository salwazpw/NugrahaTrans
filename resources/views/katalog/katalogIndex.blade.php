            ...
            <table class="table  table-primary">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{$data->merk}}</td>
                        <td>{{$data->warna}}</td>
                        <td><img width="150px" height="100px" src="{{ asset('storage/'.$data->gambarKendaraan) }}"></td>
                        <td>Rp. {{number_format($data->harga, 0, ",", ".")}}</td>
                        <td>{{$data->catatan}}</td>
                        <td>
                            <form action="{{ route('katalog.destroy',  $data->id) }}" method="POST">
                                <a class="btn btn-icons btn-primary" href="{{route('katalog.show', $data->id)}}"><i class="fa fa-eye"></i></a>
                                <a class="btn btn-icons btn-warning" href="{{route('katalog.edit', $data->id)}}"><i class="fa fa-edit"></i></a>
                                @csrf
                                @method('DELETE')
